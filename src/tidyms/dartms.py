...
class DartMSAssay:
    ...
    def correct_MZ_shift_across_samples(self, referenceMZs = [165.078978594 + 1.007276], 
        ...
        temp = self._calculate_mz_offsets(referenceMZs = referenceMZs, max_mz_deviation_absolute = max_mz_deviation_absolute, selection_criteria = selection_criteria)
            ...
            if transformFactor is None:
                ...
                for k, spectrum in msDataObj.get_spectra_iterator():
                    ...
                    if correctby == "mzDeviationPPM":
                        ...
                        spectrum.reverseMZ = functools.partial(self._reverse_applied_mz_offset, correctby = "mzDeviationPPM", transformFactor = transformFactor)   ## TODO chain reverseMZ lambdas here
                        spectrum.reverseMZDesc = "mzDeviationPPM by %.5f (%s)"%(transformFactor, spectrum.reverseMZ)
                    elif correctby == "mzDeviation":
                        spectrum.mz = spectrum.mz - transformFactor
                        spectrum.reverseMZ = functools.partial(self._reverse_applied_mz_offset, correctby = "mzDeviation", transformFactor = transformFactor)
            ...
            for featurei in tqdm.tqdm(range(len(tempClusterInfo["minMZ"])), desc = "bracketing: generating plots"):
                for samplei, sample in enumerate(self.assay.manager.get_sample_names()):
                    ...
                    for k, spectrum in msDataObj.original_MSData_object.get_spectra_iterator():
                        usei = np.where(np.logical_and(spectrum.original_mz >= spectrum.reverseMZ(self.features[featureInd][0]), spectrum.original_mz <= spectrum.reverseMZ(self.features[featureInd][2])))[0]
                        if usei.size > 0:
                            for i in usei:
                                temp = _add_row_to_pandas_creation_dictionary(temp, 
                                    rt        = spectrum.time,
        ...
        for grp in uGroups:
            ...
            if type(grp) == list and len(grp) == 2 and type(grp[0]) == str and type(grp[1]) == str and grp[1] == "Batch":
                for batch in list(set(self.batches)):
                    groupInd = list(set([i for i, group in enumerate(self.groups) if group == grp[0]]).intersection(set([i for i, b in enumerate(self.batches) if b == batch])))
