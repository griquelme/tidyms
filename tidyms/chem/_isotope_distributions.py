...
def _find_n_atoms_abundances(
    isotope: Isotope, n: int, max_length: int, abundance: Optional[np.ndarray] = None
    ...
    if not utils.has_unique_elements(nominal):
        nominal, exact, abundance = _merge_same_nominal_mass(nominal, exact, abundance)
    ...
    nominal, exact, abundance = _resize_to_max_length(
        nominal, exact, abundance, max_length
    )
...
def _sort_by_exact_mass(
    nominal: np.ndarray, exact: np.ndarray, abundance: np.ndarray
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
...
def _resize_to_max_length(
    nominal: np.ndarray, exact: np.ndarray, abundance: np.ndarray, max_length: int
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    size = nominal.size
    if size < max_length:
        nominal = np.hstack((nominal, np.zeros(max_length - size, dtype=int)))
        exact = np.hstack((exact, np.zeros(max_length - size, dtype=float)))
        abundance = np.hstack((abundance, np.zeros(max_length - size, dtype=float)))
...
def _add_missing_nominal(
    nominal: np.ndarray, exact: np.ndarray, abundances: np.ndarray
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
...
def _merge_same_nominal_mass(
    nominal_comb: np.ndarray, exact_comb: np.ndarray, abundance_comb: np.ndarray
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    ...
    for k in range(len(change_nominal_index) - 1):
        ...
        mean_mass = (
            abundance_comb[start:end] * exact_comb[start:end]
        ).sum() / abundance_sum
...
def _validate_abundances(abundances: np.ndarray, nominal: np.ndarray):
    ...
    if not (is_normalized and is_same_size and is_all_non_negative):
        msg = (
            "abundances must have the same shape as the nominal mass"
            "array and its sum must be equal to 1."
        )
        raise ValueError(msg)


def _abundances_from_isotope(
...
def _combine_element_abundances(
    nom1: np.ndarray,
    exact1: np.ndarray,
    ab1: np.ndarray,
    nom2: np.ndarray,
...
def find_formula_abundances(
    ...
    for isotope, coeff in d.items():
        ...
        tmp_nom, tmp_ex, tmp_ab = _find_n_atoms_abundances(
            isotope, coeff, max_length, abundance=isotope_abundance
        )
        res_nom, res_ex, res_ab = _combine_element_abundances(
            res_nom, res_ex, res_ab, tmp_nom, tmp_ex, tmp_ab, min_p=min_p
        )
...
def _make_element_abundance_array(atom, n_min, n_max, length, abundance=None):
    ...
    for n in range(n_min, n_max + 1):
        tmp_nom, tmp_ex, tmp_ab = _find_n_atoms_abundances(
            atom, n, length, abundance=abundance
        )
...
def make_coeff_abundances(
    ...
    for k, isotope, b in zip(range(len(isotopes)), isotopes, bounds):
        ...
        tmp_nom, tmp_ex, tmp_ab = _make_element_abundance_array(
            isotope, lower, upper, length, abundance=tmp_abundance
        )
        ...
        res_nom, res_ex, res_ab = _combine_array_abundances(
            res_nom, res_ex, res_ab, k_nom, k_ex, k_ab, min_p=min_p
        )
    return res_nom, res_ex, res_ab
