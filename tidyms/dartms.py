...
## example script to use DARTMS capabilities
if False:

    print("First test of importing tidyMS on a new miniconda environment for the processing of DART-MS datasets")

    import sys
    sys.path.append("D:/Projects/DARTMS/tidyms")
    ...
    ## Specify data to be loaded
    path = "D:/Projects/DARTMS/Serum_DARTMS/mzML_profileMode/"; ms_mode = "profile"
    path = "D:/Projects/DARTMS/Serum_DARTMS/mzML/"; ms_mode = "centroid"
    instrument = "qtof"
    filenames = [os.path.join(path, file) for file in os.listdir(path) if file.endswith(".mzML")][1:4]

    if False and os.path.exists("./temp_assay.pickle") and os.path.isfile("./temp_assay.pickle") and os.path.exists("./temp_results.pickle") and os.path.isfile("./temp_results.pickle"):
        print("Loading previous results from cache files")
        print("")
        with open("./temp_assay.pickle", "rb") as fin:
            assay = pickle.load(fin)

        with open("./temp_results.pickle", "rb") as fin:
            samples, sample_metadata, groups, batches, features, dat = pickle.load(fin)

    else:
        ## Import chronograms and separate them into spots. 
        print("Importing and separating chronograms")
        assay = ms.dartms.create_assay_from_chronogramFiles(filenames, ms_mode = ms_mode, instrument = instrument, spot_file = "./Serum_DARTMS/defined_spots.tsv", centroid_profileMode = True,
                                                            fileNameChangeFunction = lambda x: x.replace("20220316-HE-350V-400C-POS-", "").replace("20220316-HE-350V-500C-POS-", ""))
        print("")

        ## Calculate concensus spectra for each spot file to reduce following processing time
        print("Collapsing multiple spectra of samples to concensus spectra for each sample")
        ms.dartms.calculate_concensus_spectra_per_sample(assay, min_difference_ppm = 100, min_signals_per_cluster = 5, 
                                                        cluster_quality_check_functions = [
                                                        ...
                                                        aggregation_function = "sum", featureMLlocation = "./Serum_DARTMS/mzML/")
        print("")

        ## Account for and correct mz values of individual spots by reference mz values
        print("Correcting for mz shifts between samples (not chronograms) with reference mz values")
        referenceMZs = sorted([149.03317, 192.15114, 279.17694, 338.355, 462.17768, 536.20127, 603.56, 758.27174, 875.83, 1197.16, 1017.97])
        _, p = ms.dartms.correct_MZ_shift_across_samples(assay, referenceMZs, max_mz_deviation_absolute = 0.1, correctby = "mzDeviationPPM", plot = True)
        print(p)
        print("")

        ## Normalize to internal standard
        print("Normalize to internal standard")
        std = (316.29, 316.325)
        ms.dartms.normalize_to_internal_standard(assay, std, multiplication_factor = 1E6, plot = True)
        print("")

        ## Bracket/group mz features across samples of the experiment
        print("Bracketing features across samples")
        brackRes = ms.dartms.bracket_samples(assay, max_ppm_deviation = 25)
        ms.dartms.write_brac_results_to_featureML(brackRes, featureMLlocation = "./Serum_DARTMS/mzML/bracketedResults.featureML", featureMLStartRT = 0, featureMLEndRT = 1400)
        print("   .. bracketed to %d features"%(len(brackRes)))
        print("")

        #TODO implement re-integration strategy here 

        print("Building data matrix")
        samples, features, dat = ms.dartms.build_data_matrix(assay, brackRes)
        print("   .. data matrix consists of %d samples and %d features"%(len(samples), len(features)))
        print("")

        ## Fetch metadata
        sample_metadata = assay.get_sample_metadata()
        groups = [sample_metadata.loc[sample, "group"] for sample in samples]
        batches = [sample_metadata.loc[sample, "batch"] for sample in samples]
        
        ## Save assay to file for quick prototyping
        with open("./temp_assay.pickle", "wb") as fout:
            pickle.dump(assay, fout)

        ## Save assay to file for quick prototyping
        with open("./temp_results.pickle", "wb") as fout:
            pickle.dump([samples, sample_metadata, groups, batches, features, dat], fout)


    np.savetxt("./foo.csv", dat)

    print("Annotate features")
    annotations = ms.dartms.annotate_features(dat, features, samples)
    print("The following annotations have been extracted: ")
    temp = {x: annotations.count(x) for x in annotations}
    for k in sorted((k for k in temp), key = lambda x: temp[x], reverse = True)[0:25]:
        print("   .. '%10s': %4d times"%(k, temp[k]))
    print("")

    ## Blank subtraction with QC sample
    print("Remove features that are likely only the background")
    oFeatures = dat.shape[1]
    keeps = ms.dartms.blank_subtraction(dat, features, groups, "blank", ["QC", "ccRCC", "Cntrl"], foldCutoff = 2, plot = True)
    keeps = [i for i in range(len(keeps)) if keeps[i]]
    dat = dat[:, keeps]
    features = [features[i] for i in keeps]
    ms.dartms.write_brac_results_to_featureML(features, featureMLlocation = "./Serum_DARTMS/mzML/bracketedResults_afterBlankSubtraction.featureML", featureMLStartRT = 0, featureMLEndRT = 1400)
    print("   .. of the %d features %d (%.1f%%) will be kept"%(oFeatures, len(keeps), len(keeps) / oFeatures * 100))
    print("")


    ## Show brief overview of the results
    print("Overview of results")
    ms.dartms.print_results_overview(dat, groups)
    print("")


    ## RSD plots
    print("RSD of groups")
    p = ms.dartms.plot_RSDs_per_group(dat, groups, type = "points")
    print(p)


    ## Remove blank groups
    print("Remove blank groups")
    blankGroups = ["blank"]
    samples, groups, batches, dat = ms.dartms.remove_blank_groups(samples, groups, batches, dat, blankGroups)


    ## Volcano plot
    if True:
        volcanoPlots = [["ccRCC", "Cntrl"], ["ccRCC", "QC"], ["Cntrl", "QC"]]
        ...
        for grp1, grp2 in volcanoPlots:
            ...
            for featurei in range(dat.shape[1]):
                ...
                if np.all(valsGrp1 == 0) and np.all(valsGrp2 == 0):
                    ...
                    sigInd = "sig" if pval <= 0.05 and (fold > 2 or fold < 1/2) else "-"

                    pvals.append(-np.log10(pval))
                    folds.append(np.log2(fold))
                    effectSizes.append(cohen)
                    meanAbundances.append(meanAbundance)
                    sigInds.append(sigInd)
                    tests.append("'%s' vs '%s'"%(grp1, grp2))

        temp = {"pvalues": pvals, "folds": folds, "effectSizes": effectSizes, "meanAbundances": meanAbundances, "sigIndicators": sigInds, "tests": tests}
        temp = pd.DataFrame(temp)
        p = (p9.ggplot(data = temp, mapping = p9.aes(
                x = "folds", y = "pvalues", colour = "sigIndicators"
            ))
            + p9.geom_hline(yintercept = -np.log10(0.05), alpha = 0.3, colour = "slategrey")
            + p9.geom_vline(xintercept = [np.log2(2), np.log2(1/2)], alpha = 0.3, colour = "slategrey")
            + p9.geom_point(alpha = 0.3)
            + p9.facet_wrap("~tests")
            + p9.theme_minimal()
            + p9.theme(legend_position = "bottom")
    ...
    if True:
        ...
        #datImp = dat[:,~np.isnan(dat).any(axis = 0)]
        ...
        scores = pca.fit(datImpSca).transform(datImpSca)

        ## plot 
        temp = {"pc1": scores[:,0], "pc2": scores[:,1], "file": samples, "group": groups, "batch": batches}
        temp = pd.DataFrame(temp)
        temp['batch'] = temp['batch'].astype(object)
        p = (p9.ggplot(data = temp, mapping = p9.aes(
                x = "pc1", y = "pc2", colour = "group", group = "group"
            ))
            + p9.stat_ellipse(alpha = 0.5, level = 0.95)
            + p9.geom_point(alpha = 0.8)
            + p9.theme_minimal()
            + p9.theme(legend_position = "bottom")
            + p9.theme(subplots_adjust={'wspace':0.15, 'hspace':0.25, 'top':0.93, 'right':0.99, 'bottom':0.15, 'left':0.15})
            + p9.ggtitle("PCA of dataset with %d features"%(datImpSca.shape[1]))
        )
        print(p)

        p = (p9.ggplot(data = temp, mapping = p9.aes(
                x = "pc1", y = "pc2", colour = "batch", group = "batch"
            ))
            + p9.stat_ellipse(alpha = 0.5, level = 0.95)
            + p9.geom_point(alpha = 0.8)
            + p9.theme_minimal()
    ...
    if True:
        ...
        embedding = reducer.fit_transform(datImp)

        ## plot 
        temp = {"pc1": embedding[:,0], "pc2": embedding[:,1], "file": samples, "group": groups, "batch": batches}
        temp = pd.DataFrame(temp)
        temp['batch'] = temp['batch'].astype(object)
        p = (p9.ggplot(data = temp, mapping = p9.aes(
                x = "pc1", y = "pc2", colour = "group", group = "group"
