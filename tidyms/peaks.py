...
# TODO: detector method should use find_peaks or find_peaks_cwt from
# scipy
...
class PeakLocation:
    ...
    def get_loc(self, x: np.array, y: np.array, baseline: np.array):
        ...
        weights[weights < 0] = 0
        loc = np.abs(np.average(x[self.start:(self.end + 1)], weights=weights))
        return loc
...
def _estimate_local_noise(x: np.ndarray, robust: bool = True) -> float:
    ...
    # size of d2x. Using the 95 % IC, the quotient between the mean and
    # standard deviation should be lower than 2 / sqrt(n - 2). This is the
    # criteria used to find the best noise value.
    # d2x with high absolute values are removed until this
    # condition is meet. start at 90th percentile and decrease it in each
    # iteration. The loop stops at the 20th percentile even if this condition
    # is not meet
    n_deviations, dev_threshold = 3, 1    # dummy values to initialize the loop
    percentile_counter = 9  # start at 90th percentile
    noise_std = 0
    while (n_deviations > dev_threshold) and (percentile_counter > 2):
        percentile = percentile_counter * d2x.size // 10
        ...
        if robust:
            noise_std = mad(d2x[:percentile], scale="normal")
...
def estimate_noise(x: np.ndarray, min_slice_size: int = 200,
    ...
    n_slices: int, optional
        Number of slices to create. The size of the slice must be greater than
        `min_slice_size`.
    robust : bool
        If True, estimates the noise using the median absolute deviation. Else
        uses the standard deviation.
...
def smooth(x: np.ndarray, window: Callable, *window_args,
           **window_kwargs) -> np.ndarray:
    """
    ...
    ----------
    x: 1D array
    window : function used to create the window.
        It accepts any function in scipy.signal.windows.
    window_args : other arguments to pass to the window function
    window_kwargs : keyword arguments to pass to the window function
    ...
    """
    window_array = window(x.size, *window_args, **window_kwargs)
    window_array /= window_array.sum()
    # g = gaussian(x.size, strength)
    # g /= g.sum()
    x_smoothed = np.convolve(x, window_array, "same")
...
def estimate_baseline(x: np.ndarray, noise: np.ndarray, min_proba: float = 0.05,
                      return_index=False):
    """
    Estimate the baseline of a 1D signal by classifying each point as peak or
    baseline.
    ...
    Notes
    -----
    The baseline is built as follows: First, each point in the signal is
    classified as either peak or baseline. Using the baseline points, the
    baseline for the signal is built using linear interpolation.

    To classify points as peak or baseline, the signal is divided into slices
    defined by local maxima and local minima of the signal. The difference
    between the maximum and minimum in each slice is compared against the noise
    in the slice and the probability of obtaining a difference greater than the
    observed with the noise is computed assuming that the noise is gaussian iid.
    If the probability is lower than `min_proba`, the slice is regarded as
    baseline.
...
def _find_baseline_points(x: np.ndarray, noise: np.ndarray,
    ...
    extrema = _find_local_extrema(smoothed)
    # check how likely is that the difference observed in each min-max interval
    # can be attributed to noise.
    noise_probability = _estimate_noise_probability(noise, smoothed, extrema)
...
def _estimate_noise_probability(noise, x, extrema):
    ...
    extrema_reduce_at = (np.vstack([extrema, np.roll(extrema, -1) + 1])
                         .T.reshape(extrema.size * 2)[:-2])
    extrema_reduce_at[-1] -= 1  # prevents an IndexError in the last slice
    # the standard deviation of the sum of the noise in each slice
    noise_slice = np.sqrt(np.add.reduceat(noise ** 2, extrema_reduce_at)[::2])
...
def _build_baseline_index(x: np.ndarray, noise: np.ndarray,
                          noise_probability: np.ndarray, min_p, extrema):
...
#         y = smooth(y, gaussian, smoothing_strength)
#
#     if baseline is None:
...
def detect_peaks(x: np.ndarray, baseline_index: np.ndarray, method: str,
                 find_peaks_params) -> List[PeakLocation]:
    r"""
    Finds peaks in a 1D signal.
    ...
    x : array
    method : {"max", "cwt"}
        Method used to detect peaks. "max" uses
        :py:function:`scipy.signal.find_peaks`. and "cwt" uses
        :py:function:`scipy.signal.find_peaks_cwt`
    find_peaks_params : dict, optional
        parameters to pass to the `find_peaks` function.
    baseline_index: baseline indices obtained with estimate_baseline
    ...
    1.  Local maxima are found in the signal using the selected peak picking
        function. Peaks are conserved only if they are in a region classified
        as peak using the `baseline_index` parameter.
    2.  For each peak, the extension is computed by searching the closest
        baseline point to the left and right.
    3.  If there are overlapping peaks (i.e. overlap in the peak extension),
    ...
    if method == "max":
        ...
        # remove peaks parameters estimated by find_peaks
        if isinstance(peaks, tuple):
            peaks = peaks[0]
    elif method == "cwt":
        peaks = find_peaks_cwt(x, **find_peaks_params)
    else:
        msg = "Valid methods are `max` or `cwt`"
        raise ValueError(msg)

    # keep peaks not in the baseline
    peaks = np.setdiff1d(peaks, baseline_index, assume_unique=True)
...
def _fix_peak_overlap(y: np.ndarray, start: np.ndarray, peaks: np.ndarray,
    ...
    for k in overlap_index:
        ...
        k_min = np.argmin(local_min[ks:ke])
...
def gaussian_wavelet(size, width, window_size=None):
    return gaussian(size, width)
