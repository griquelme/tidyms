...
def lc_sample() -> ms.core.SampleData:
    tidyms_path = ms.fileio.get_tidyms_path()
    file_path = Path(tidyms_path) / "test-nist-raw-data/NZ_20200227_039.mzML"
    sample = ms.core.Sample(file_path, file_path.stem)
    return ms.core.SampleData(sample)
...
def pipeline():
    ...
    pipeline = ms.core.ProcessingPipeline(processing_steps)
...
def test_Assay_add_sample(pipeline: ms.core.ProcessingPipeline, tmp_path: Path):
    tidyms_path = ms.fileio.get_tidyms_path()
    file_path = Path(tidyms_path) / "test-nist-raw-data/NZ_20200227_039.mzML"
    sample = ms.core.Sample(file_path, file_path.stem)
    data_path = tmp_path / "assay-data"
    assay = ms.core.Assay(data_path, pipeline, ms.lcms.LCTrace, ms.lcms.Peak)
    assay.add_samples([sample])


def test_lc_process_sample(pipeline: ms.core.ProcessingPipeline, tmp_path: Path):
    tidyms_path = ms.fileio.get_tidyms_path()
    file_path = Path(tidyms_path) / "test-nist-raw-data/NZ_20200227_039.mzML"
    sample = ms.core.Sample(file_path, file_path.stem)
    data_path = tmp_path / "assay-data"
