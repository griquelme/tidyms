...
from tidyms.lcms import Peak, LCTrace, _overlap_ratio, _get_overlap_index, _compare_features_lc
...
def test__overlap_ratio_overlapping_peaks():
    ...
    roi1 = LCTrace(scans_roi1, scans_roi1, scans_roi1, scans_roi1)
    roi2 = LCTrace(scans_roi2, scans_roi2, scans_roi2, scans_roi2)
    ft1 = Peak(5, 10, 15, roi1)
    ft2 = Peak(5, 10, 15, roi2)
    test_result = _overlap_ratio(ft1, ft2)
    expected_result = 0.5
...
def test__overlap_ratio_perfect_overlap():
    ...
    roi1 = LCTrace(scans_roi1, scans_roi1, scans_roi1, scans_roi1)
    roi2 = LCTrace(scans_roi2, scans_roi2, scans_roi2, scans_roi2)
    ft1 = Peak(10, 15, 20, roi1)
    ft2 = Peak(0, 5, 10, roi2)
    test_result = _overlap_ratio(ft1, ft2)
    expected_result = 1.0
    assert isclose(expected_result, test_result)
...
def test__get_overlap_index_partial_overlap():
    ...
    ft1 = Peak(5, 10, 15, roi1)
    ft2 = Peak(5, 10, 15, roi2)
    test_result = _get_overlap_index(ft1, ft2)
...
def test__feature_similarity_same_features():
    ...
    ft1 = Peak(10, 15, 20, roi1)
    min_overlap = 0.5
    test_result = _compare_features_lc(ft1, ft1)
    expected_result = 1.0
    assert isclose(expected_result, test_result)
...
def test__feature_similarity_non_overlapping_features():
    ...
    roi2 = LCTrace(scans_roi2, scans_roi2, scans_roi2, scans_roi2)
    ft2 = Peak(10, 15, 20, roi2)
    min_overlap = 0.5
    test_result = _compare_features_lc(ft1, ft2)
    expected_result = 0.0
    assert isclose(expected_result, test_result)
...
def annotator():
    ...
    return annotation.create_annotation_tools(**params)
