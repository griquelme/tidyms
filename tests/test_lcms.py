...
def test_extend_roi(roi_data):
    rt, mz, spint = roi_data
    roi = lcms.Roi(spint, mz, rt, 50)
    roi.extend(2)
    assert roi.mz.size == (mz.size + 4)
...
def test_make_roi(simulated_experiment):
    roi_list = lcms.make_roi(simulated_experiment, 0.005, 0, 0, 0, "reduce")
...
def test_make_roi_min_intensity(simulated_experiment):
    min_intensity = 15
    roi_list = lcms.make_roi(simulated_experiment, 0.005, 0, 0, min_intensity,
                             "reduce")
...
def test_make_roi_start(simulated_experiment):
    start = 10
    roi_list = lcms.make_roi(simulated_experiment, 0.005, 0, 0, 0,
                             "reduce", start=start)
    n_sp = simulated_experiment.getNrSpectra()
    for r in roi_list:
...
def test_make_roi_multiple_match_reduce_merge(simulated_experiment):
    ...
    tolerance = 31
    roi_list = lcms.make_roi(simulated_experiment, tolerance, 0, 0, 0,
                             "reduce", targeted_mz=targeted_mz)
...
def test_make_roi_multiple_match_reduce_custom_sp_reduce(simulated_experiment):
    ...
    roi_list = lcms.make_roi(simulated_experiment, 0.005, 0, 0, 0,
                             "reduce", sp_reduce=sp_reduce)
    assert len(roi_list) == simulated_experiment.mz_params.shape[0]


def test_make_roi_invalid_multiple_match(simulated_experiment):
    with pytest.raises(ValueError):
        lcms.make_roi(simulated_experiment, 0.005, 0, 0, 0,
                      "invalid_mode")
